...
import { handleAddItem, handleRemoveItem } from '../../controller/cart';
import fidnDataById from '../../model/find-data';
import { setItemCount } from '../../model/cart';

export default function createCartItem(num: number, data: Product) {
  const item = createElement('div', 'content__item item');
  ...
  const itemStock = createElement('p', 'item__stock', `Stock: ${itemDefaultStock}`);
